...
The Rossmann Store Sales dataset contains 1,017,209 daily sales records from 1,115 German drugstores (January 2013 -- July 2015), joined with store metadata. Records with closed stores or zero sales were excluded, leaving 844,338 samples. An 80/20 split (seed~42) yielded 675,470 training and 168,868 validation samples. The target variable has mean~\euro6,955.96, standard deviation~\euro3,103.82, and skewness~1.59.
...
Table~\ref{tab:shap} reports mean absolute SHAP values for the top eight features; Fig.~\ref{fig:beeswarm} shows the beeswarm summary plot.
...
Fig.~\ref{fig:waterfall} shows the waterfall plot for Store~262, a Wednesday in June 2026 with an active promotion. Starting from the dataset baseline of \euro6,955, the promotion contributes $+$\euro1,247; competitor proximity ($\approx$310~m) subtracts \euro698; a spring seasonal effect adds \euro412; a mid-week adjustment adds \euro180, yielding \euro8,786. A manager can read this directly: the promotional benefit is quantified, the competitive penalty is explicit, and the effect of removing the promotion ($-$\euro1,247) is immediately apparent. These are decision-relevant attributions, not abstract model properties.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{shap_waterfall.png}
    \caption{SHAP waterfall plot — Store~262, June 2026 (promotional day). Baseline \euro6,955; prediction \euro8,786.}
    \label{fig:waterfall}
\end{figure}

Applying the model to 2026 store profiles confirms the seasonal structure in the training data: January is the annual trough at an estimated \euro7,480/store/day; December peaks at \euro10,883 — a 45.5\% uplift — consistent with the Christmas surge observed across all three training years. Absolute values are indicative only, as they do not account for post-2015 inflation or network changes.
...
This study benchmarked Linear Regression, Random Forest, and XGBoost on the Rossmann dataset and applied TreeSHAP to produce auditable, store-level predictions. XGBoost achieved the strongest performance ($R^{2}$~=~0.8512, MAPE~=~13.78\%), and SHAP analysis revealed that competition proximity, promotional activity, and store identity are the primary drivers of daily sales variance. The waterfall explanation for Store~262 showed that individual forecasts can be decomposed into interpretable contributions, directly supporting retail management decisions. Key limitations include the random validation split — which time-series cross-validation would address — and the restriction to a single sector and time window.

Future work should evaluate LightGBM~\cite{ke2017} and CatBoost~\cite{prokhorenkova2018}, incorporate external data sources such as weather and economic indicators, apply Bayesian hyperparameter optimisation, and validate on broader benchmarks such as the M5 or Walmart Sales datasets. The combination of strong predictive accuracy and post-hoc explainability demonstrated here provides a practical foundation for deploying machine learning in retail forecasting where transparency alongside performance is required.
